  ...
  async function fetchData(selectedUnit) {
    try {
      ...
      const [open_inv, open_rec] = await Promise.all([
        fetch(`${baseURL}/sync/openInvoice?selectedUnit=${selectedUnit}`).then(
          (response) => response.json()
        ),
        fetch(`${baseURL}/sync/getReceipts?selectedUnit=${selectedUnit}`).then(
          (response) => response.json()
        ),
      ]);
      ...
      {/* <div className="col-md-12">
        <div className="row col-md-12">
          <div className=" row col-md-8 mt-1">
            <label
              className="form-label col-md-4 mt-2"
              style={{ whiteSpace: "nowrap" }}
            >
              Syncronise Account Details
            </label>
            <select
              className="ip-select col-md-2 mb-2"
              value={selectedUnit} // Set the value of the select to the state variable
              onChange={handleSelectChange}
            >
              {units.map((item, index) => {
