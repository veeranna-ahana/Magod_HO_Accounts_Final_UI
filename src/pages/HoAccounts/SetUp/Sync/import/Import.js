...
export default function Import(props) {
  ...
  const sync_data = (parsedData) => {
    ...
    props.data.open_inv.forEach((inv) => {
      ...
      if (
        parsedData.open_inv.filter((item) => item.DC_Inv_No === inv.DC_Inv_No)
          .length === 0
      ) {
        const newInv = {
    ...
    parsedData.open_inv.forEach((inv) => {
      const filt = `Dc_Inv_No=${inv.DC_Inv_No}`;
      if (
        props.data.open_inv.filter((item) => item.DC_Inv_No === inv.DC_Inv_No)
          .length === 0
      ) {
        const newInv = {
          DC_Inv_No: inv.DC_Inv_No,
    ...
    const syncRVList = props.data.open_rec.map((horv) => {
      const unitRv = parsedData.open_rec.find(
        (unitRv) => horv.RecdPVID === unitRv.RecdPVID
      );

      if (
        unitRv &&
        (horv.ReceiptStatus !== unitRv.ReceiptStatus ||
          horv.On_account !== unitRv.On_account)
      ) {
        const newRV = {
    ...
    props.data.open_rec.forEach((rv) => {
      const filt = parsedData.open_rec.filter(
        (unitRv) => rv.RecdPVID === unitRv.RecdPVID
      );
    ...
    parsedData.open_rec.forEach((rv) => {
      const filt = props.data.open_rec.filter(
        (unitRv) => rv.RecdPVID === unitRv.RecdPVID
      );
  ...
  const postData = async () => {
    axios
      .put(baseURL + "/sync/postData", updatedataa)
      .then((res) => {
